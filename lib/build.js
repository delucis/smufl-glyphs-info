...
/**
 * Main function generating GlyphData.xml
...
function generateGlyphData () {
  ...
  for (let name in GLYPHNAMES) {
    ...
    xml.ele('glyph', {
      ...
      category: 'SMuFL Ranges',
      subCategory: GLYPH2RANGEMAP[name],
      unicode: codepoint,
      altNames: `uni${codepoint}`,
...
/**
 * Main function generating Groups.plist
...
FS.writeFile('dist/GlyphData.xml', generateGlyphData(), err => {
  if (err) {
    console.error(err)
  } else {
    console.log('Saved GlyphData.xml to disk')
  }
})

FS.writeFile('dist/Groups.plist', generatePList(), err => {
  if (err) {
    console.error(err)
  } else {
    console.log('Saved Groups.plist to disk')
  }
})
